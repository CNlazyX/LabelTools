...
labelme_path = r'E:\CODE\zyx_tools\1\images\6500.json'
    ...
    for each_ann in labelme['shapes']:  # 遍历每个标注
        ...
        if each_ann['shape_type'] == 'rectangle':  # 如果遇到框
            ...
            if description_rectangle == None:
                ...
                for each_ann in labelme['shapes']:  # 遍历所有标注
                    if (each_ann['shape_type'] == 'point') & (description_rectangle == each_ann['description']):  # 筛选出关键点标注
                        # 关键点XY坐标、类别
                        x = int(each_ann['points'][0][0])
                        y = int(each_ann['points'][0][1])
                        label = each_ann['label']
